...
func getCurrentContestStatus(qdb sqlx.Queryer) (xsuportal.ContestStatus, error) {
	...
	if contestStatus.StatusStr != "" {
		...
		switch {
		case contestStatus.CurrentTime.Before(contestStatus.RegistrationOpenAt):
			contestStatus.StatusStr = "standby"
		case contestStatus.CurrentTime.Before(contestStatus.ContestStartsAt):
			contestStatus.StatusStr = "registration"
		case contestStatus.CurrentTime.Before(contestStatus.ContestEndsAt):
...
func makeLeaderboardPB(teamID int64) (*resourcespb.Leaderboard, error) {
	...
	for idx := range pb.Teams {
		pb.Teams[idx].Scores = teamGraphScores[pb.Teams[idx].Team.Id]
	}
	return pb, nil
}
